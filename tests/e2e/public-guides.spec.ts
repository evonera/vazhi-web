...
test('profiles show only public guides and immutable guide versions can be reported', async ({ page }) => {
  await page.route('**/api/profile?handle=asha', (route) => route.fulfill({
    ...
    body: JSON.stringify({
      ...
      listings: [{ slug: 'penang-walk', title: 'Penang on foot', subtitle: 'A gentle old-town route.', stopCount: 2, updatedAt: 1 }],
    }),
  }))
  await page.route('**/api/listing?**', async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        title: 'Penang on foot · first edition', subtitle: 'A gentle old-town route.',
