...
/** Calculate a route from the authenticated owner's stored Path projection. */
export const forCurrentOwner: RegisteredAction<'public', { pathID: Id<'paths'>; travelMode: TravelMode }, Promise<RouteResult>> = action({
  ...
  handler: async (ctx, args): Promise<RouteResult> => {
    ...
    const stored: { routeRevision: number; stops: Array<{ placeId: string } | { latitude: number; longitude: number }> } =
      await ctx.runQuery(internal.routes.storedStopsForOwner, { ownerAuthUserId, pathId: args.pathID })
    await ctx.runMutation(internal.routes.reserveOwnerRouteCalculation, { ownerAuthUserId })
    const result: RouteResult = await ctx.runAction(internal.routes.compute, {
      stops: stored.stops,
      travelMode: args.travelMode,
    })
