...
// Prints variable names only. Run through the provider's secret-injected
// environment; never paste production values into a terminal command.
...
const convexRequired = [
  ...
  'MODERATION_API_TOKEN', 'RATE_LIMIT_SALT', 'EDGE_INGRESS_SIGNING_SECRET',
]
const workerRequired = [
  'CONVEX_HTTP_URL', 'PUBLIC_WEB_ORIGIN', 'TURNSTILE_SECRET_KEY',
  'RATE_LIMIT_SALT', 'EDGE_INGRESS_SIGNING_SECRET', 'VITE_TURNSTILE_SITE_KEY',
]
...
const sections = [
  ...
  ...(mode === 'commerce' && (target === 'convex' || target === 'all')
    ? [['RevenueCat Web', missing(commerceRequired)]]
    : []),
]
