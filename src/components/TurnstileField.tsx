...
export function TurnstileField({ onToken }: { onToken: (token: string | null) => void }) {
  ...
  useEffect(() => {
    if (!siteKey) {
      setMessage('Safety verification is not configured for this link yet.')
      return
    }
    let cancelled = false
    const render = () => {
      ...
      widgetID.current = window.turnstile.render(container.current, {
        ...
        'error-callback': () => {
          onToken(null)
          setMessage('Safety verification did not load. Please try again.')
        },
        'expired-callback': () => {
          onToken(null)
          setMessage('Safety verification expired. Please complete it again.')
        },
      })
    }
  ...
  return (
    <div className="turnstile-field">
      <div ref={container} />
      <p aria-live="polite">{message ?? 'Complete the safety check to send your recommendation.'}</p>
    </div>
  )
}
