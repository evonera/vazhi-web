...
export async function edgeIngressSignature(rawBody: string, signingSecret: string, timestamp = Math.floor(Date.now() / 1_000)) {
...
export async function verifyEdgeIngressSignature({ rawBody, signatureHeader, signingSecret, nowSeconds = Math.floor(Date.now() / 1_000) }: {
  rawBody: string
