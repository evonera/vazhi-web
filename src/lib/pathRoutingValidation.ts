...
export function validatePrivatePathRouteInput(input: {
  ...
  for (const stop of input.stops) {
    if (!stop.localStopID || stop.localStopID.length > 80 || stopIDs.has(stop.localStopID) || orders.has(stop.orderIndex)) {
    ...
    if (stop.place.provider === 'google') {
      if (!stop.place.providerPlaceID.trim() || stop.place.providerPlaceID.length > 512) {
        return 'Choose a Google place before calculating a route.'
      }
