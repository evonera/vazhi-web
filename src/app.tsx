...
function RecommendationForm({ request, demo }: { request: PublicAskRequest; demo: boolean }) {
  ...
  return <form className="recommendation-card" onSubmit={submit}>
    ...
    <fieldset><legend>The good stuff</legend>
      ...
      {(selectedPlace?.provider === 'google' || results.some((place) => place.provider === 'google')) && <p className="maps-attribution" aria-label="Google Maps attribution">Google Maps</p>}
...
function ProPage() {
  ...
  return <main className="pro-page"><SiteHeader /><section className="pro-page__content"><p className="section-label">VAZHI PRO</p><h1>More room for every path.</h1><p className="lede">Keep unlimited Paths in your private travel journal. Your subscription works with the same Vazhi account on iPhone and web.</p><div className="pro-card"><p className="section-label">WEB CHECKOUT</p><h2>Continue with your Vazhi account.</h2><p>Sign in with Apple first. RevenueCat securely presents the available plans and checkout; Vazhi does not collect your card details.</p><button className="button button--dark" type="button" onClick={() => void continueToCheckout()} disabled={status === 'loading'}>{status === 'loading' ? 'Opening checkout…' : 'Continue to secure checkout'} <span aria-hidden="true">↗</span></button>{status === 'sign-in' && <p className="pro-card__notice" role="status">{message ?? 'Sign in with Apple, then continue here.'} <a href="/sign-in?next=%2Fpro">Sign in</a></p>}{status === 'unavailable' && <p className="pro-card__notice" role="status">Web checkout is not available yet. You can still subscribe in the iPhone app.</p>}</div><p className="pro-page__fineprint">Purchases, renewals, cancellation, and refunds are managed by the checkout provider. The iPhone app checks your active Vazhi Pro entitlement through RevenueCat.</p></section><SiteFooter /></main>
}
function SignInPage() { const [error, setError] = useState<string | null>(null); const returnPath = ownerSignInReturnPath(new URLSearchParams(window.location.search).get('next')); return <main className="route-page route-page--centered"><p className="eyebrow">Owner access</p><h1>Sign in on Vazhi.</h1><p>Use Apple to manage your Vazhi account. Audience members never need an account.</p><button className="button" onClick={() => startAppleSignIn(returnPath).catch((reason: Error) => setError(reason.message))}>Continue with Apple</button>{error && <p className="form-error" role="alert">{error}</p>}</main> }
type OwnerRequest = { id: string; slug: string; prompt: string; destination: string; status: 'open' | 'closed'; recommendationCount: number }
function RequestsPage() { const [requests, setRequests] = useState<OwnerRequest[]>([]); const [error, setError] = useState<string | null>(null); const [loading, setLoading] = useState(true); async function load() { try { const token = await getConvexAccessToken(); const base = convexHTTPURL; const response = await fetch(`${base}/api/owner/ask-requests`, { headers: { Authorization: `Bearer ${token}` } }); if (!response.ok) throw new Error('Your requests are unavailable.'); setRequests(await response.json() as OwnerRequest[]) } catch (reason) { setError(reason instanceof Error ? reason.message : 'Sign in to view your requests.') } finally { setLoading(false) } }; useEffect(() => { void load() }, []); async function closeRequest(requestID: string) { try { const token = await getConvexAccessToken(); const base = convexHTTPURL; const response = await fetch(`${base}/api/owner/ask-requests`, { method: 'PATCH', headers: { 'content-type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ requestID, status: 'closed' }) }); if (!response.ok) throw new Error('The link could not be closed.'); setRequests((current) => current.map((item) => item.id === requestID ? { ...item, status: 'closed' } : item)) } catch (reason) { setError(reason instanceof Error ? reason.message : 'The link could not be closed.') } }; if (loading) return <main className="route-page route-page--centered"><p>Loading your requests…</p></main>; if (error && requests.length === 0) return <main className="route-page route-page--centered"><p className="eyebrow">Owner access</p><h1>Sign in to see your requests.</h1><p>{error}</p><a className="button" href="/sign-in">Continue with Apple</a></main>; return <main className="dashboard-page"><SiteHeader /><div className="dashboard-page__shell"><p className="eyebrow">Ask the Way</p><h1>Your requests</h1><p className="lede">Your iPhone remains the home for accepting recommendations and building a Path.</p>{error && <p className="form-error" role="alert">{error}</p>}<section className="request-list" aria-label="Your Ask the Way requests">{requests.length === 0 ? <p>No requests yet. Create one from a Journey in Vazhi.</p> : requests.map((request) => <article className="request-card" key={request.id}><p className="status"><span aria-hidden="true">{request.status === 'open' ? '●' : '○'}</span> {request.status}</p><h2>{request.destination}</h2><p>{request.prompt}</p><p className="fine-print">{request.recommendationCount} recommendation{request.recommendationCount === 1 ? '' : 's'} · <a href={`/ask/${request.slug}`}>Open public link</a></p>{request.status === 'open' && <button className="text-link" onClick={() => void closeRequest(request.id)}>Close link</button>}</article>)}</section></div><SiteFooter /></main> }
function LegalPage({ title }: { title: string }) { return <main className="legal-page"><SiteHeader /><section><p className="section-label">VAZHI TRUST</p><h1>{title}</h1><p>Vazhi is private by default. Shared Ask the Way links expose only the owner-approved prompt and destination. Recommendations are visible only to the request owner. Profiles and Path guides appear publicly only after their owner explicitly publishes them.</p>{title === 'Privacy' && <p>When an owner chooses to calculate a Path route, Vazhi stores Google Place IDs in the owner’s private cloud Path; Apple/manual pins store their user-selected coordinates. The route request sends place IDs or Apple/manual coordinates to Google Routes. Private notes, source Moments, photos, and audio are not sent. Route summaries expire after five minutes.</p>}{(title === 'Privacy' || title === 'Terms') && <p>Vazhi uses Google Maps Platform for place search and route estimates. Review the <a href="https://cloud.google.com/maps-platform/terms" rel="noreferrer">Google Maps Platform Terms</a> and <a href="https://policies.google.com/privacy" rel="noreferrer">Google Privacy Policy</a>.</p>}<p>Do not submit private addresses, sensitive locations, or material you do not have permission to share.</p></section><SiteFooter /></main> }
